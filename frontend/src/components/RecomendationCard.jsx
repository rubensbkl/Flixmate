...
export default function MovieCard({ movie, activeIcons, toggleIcon, isMobile }) {
  ...
  return (
    <div 
      className={`flex items-center bg-white shadow-sm md:shadow-md rounded-lg ${padding} movie-card`}
    >
      {/* Poster do filme */}
      <img
      ...
      <div className="flex-1 min-w-0 mx-2 md:mx-4 overflow-hidden">
        <h2 className={`${titleSize} font-bold line-clamp-1 md:line-clamp-2 movie-title`} title={movie.title}>
          {movie.title}
        </h2>
        <p className="text-xs text-gray-500">
          {movie.release_date ? new Date(movie.release_date).getFullYear() : 'N/A'}
        </p>
        <p className="text-xs text-gray-400 line-clamp-1 genre-list">
